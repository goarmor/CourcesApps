...
public class MoviesActivity extends AppCompatActivity implements MoviesAdapter.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        if (savedInstanceState != null) System.out.println("savedISnonNull");
        if (savedInstanceState == null) System.out.println("savedISisNull");
        ...
        movieService = ApiFactory.getMoviesService();
        if (savedInstanceState != null) {
            Realm realm = Realm.getDefaultInstance();
            RealmResults<Movie> results = realm.where(Movie.class).findAll();
            observable = Observable.just(realm.copyFromRealm(results))
                    .doOnSubscribe(loadingView::showLoadingIndicator)
                    .doAfterTerminate(loadingView::hideLoadingIndicator)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            mMoviesSubscription = observable.subscribe(this::showMovies, throwable -> showError());
        } else {
            observable = movieService
                    .popularMovies()
